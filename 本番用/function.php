...
//================================
//セッションファイルの置き場を変更する（/var/tmp/いかに置くと30日は削除されない）
...
//    array_shift関数は配列の先頭を取り出す関数です。クエリ結果は配列形式で入っているので、array_shift1つ目だけ取り出して判定します
...
function dbConnect(){
//  DBへの接続準備
  $dsn = 'mysql:dbname=kiwatchi_monojirou_db;host=mysql1023.db.sakura.ne.jp;charset=utf8';
  $user = 'kiwatchi';
  $password = 'orange1212';
  $options = array(
...
//    DBへ接続
    $dbh = dbConnect();
//    SQL文作成
    $sql = 'SELECT m.id AS m_id, product_id, bord_id, send_date, to_user, from_user, sale_user, buy_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b ON b.id = m.bord_id WHERE b.id = :id AND b.delete_flg = 0 ORDER BY send_date ASC';
    $data = array(':id' => $id);
//    クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
...
function appendGetParam($arr_del_key = array()){
  if(!empty($_GET)){
    ...
    foreach($_GET as $key => $val){
      if(!in_array($key,$arr_del_key,true)){ //取り除きたいパラメータじゃない場合にurlにくっるけるパラメータを生成　
        $str .= $key.'='.$val.'&';
      }
    }
